...
export const projects: Project[] = [
  {
    ...
    caseStudy: {
      ...
      architectureCards: [
        ...
        { label: "API Gateway", detail: "Node.js/Express backend — proxies ML backend, aggregates news from 10+ sources, manages WebSocket prices", icon: "🔌" },
        { label: "ML Backend", detail: "FastAPI (Python) — serves predictions, runs SHAP analysis, and generates AI-powered plain-English explanations", icon: "🧠" },
        { label: "Database", detail: "MongoDB Atlas for all persistent data — predictions, sentiment, features, explanations across 9+ collections", icon: "🗄️" },
      ...
      howItWorks: [
        ...
        { title: "Check Our Work", description: "The evaluation script compares the last 60 days of predictions to actual prices. The drift monitor checks if predictions are drifting or features losing power. A quality gate enforces ≥80% ticker coverage and ≤20% data failure rate — no silent degradation." },
      ],
      challenges: [
        ...
        "Ensuring pipeline reliability — added quality gates that fail the job if <80% of tickers are predicted or >20% data fetches fail, preventing silent degradation",
        "Real-time price streaming — Finnhub WebSocket with in-memory price cache (60s TTL), volume tracking, and automatic reconnection on disconnect",
      ],
      results:
  ...
  },
  {
    ...
    caseStudy: {
      ...
      architecture:
        "Flask monolith serving a responsive vanilla JS frontend. Dual-database support — SQLite for local development, PostgreSQL (Supabase) for production. The AI layer uses DeepSeek R1 via OpenRouter for both receipt parsing and spending insights, with a background threading pattern to handle 60-90s response times within Render's 30s proxy timeout.",
      architectureCards: [
        { label: "Backend", detail: "Flask 3.0 with Gunicorn — handles auth, expense CRUD, OCR processing, AI integration, and CSV export", icon: "🐍" },
        { label: "Database", detail: "SQLite (local) / PostgreSQL via Supabase (production) — users, expenses, categories, and AI insights tables", icon: "🗄️" },
        { label: "OCR Engine", detail: "Dual pipeline — Veryfi cloud API for speed, Tesseract local fallback for offline reliability", icon: "📷" },
        { label: "AI Layer", detail: "DeepSeek R1 via OpenRouter — parses receipt text into structured JSON and generates personalized spending insights", icon: "🧠" },
        { label: "ML Classifier", detail: "Keyword-based auto-categorizer that trains on your data — learns new category associations from every expense you add", icon: "🏷️" },
        { label: "Analytics", detail: "Chart.js 4.x dashboards — interactive doughnut, bar, and line charts for category, monthly, and daily spending", icon: "📊" },
      ],
      howItWorks: [
        ...
        { title: "Smart Categorization", description: "A keyword-based ML classifier predicts the expense category based on vendor name and description. It trains on every expense you add — the more you use it, the smarter it gets at categorizing your spending." },
